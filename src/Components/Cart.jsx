...
const Cart = () => {
  ...
  return (
    <div className="ml-[10rem] -z-10 absolute top-[10vw]  h-auto w-[89vw] px-10 py-10 bg-gray-200">
      <div
        onClick={() => navigate(-1)}
        className="cursor-pointer transition-all duration-300 hover:bg-gray-300  w-fit text-[25px] m-3 px-4 py-2 rounded-full"
      >
      ...
      {cartItems.length > 0 ? (
        cartItems.map((item) => (
          <div className="relative flex px-20 gap-3 justify-evenly bg-gray-600 py-3 rounded-xl my-3">
            ...
            <div className="text-white w-1/2 flex flex-col items-start justify-center">
                <p><span className="font-semibold text-lime-500">Products:</span> {item?.title}</p>
                <p><span className="font-semibold text-lime-500">Price:</span> ₹{Math.round(item?.price)}</p>
                <p><span className="font-semibold text-lime-500">Category:</span> {item?.category}</p>
                <p> <span className="font-semibold text-lime-500">Description: </span> {item?.description}</p>
        ...
        <div className="bg-lime-400 flex h-[20vw] items-center justify-center rounded-xl">
          {" "}
